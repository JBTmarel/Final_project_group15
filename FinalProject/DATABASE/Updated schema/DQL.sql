...
WITH combined_measurements AS (
    ...
    JOIN raforka_updated.station s ON w.power_plant_source_id = s.id
    WHERE w.power_plant_source_id IS NOT NULL
)
SELECT
    power_plant_source,
    measurement_type,
...
FROM raforka_updated.withdraws_from w
JOIN raforka_updated.customer c ON c.id = w.customer_id
JOIN raforka_updated.station s ON s.id = w.power_plant_source_id
...
JOIN raforka_updated.power_plant pp ON pp.power_plant_id = mp.power_plant_id
JOIN raforka_updated.station s ON s.id = pp.power_plant_id
JOIN monthly_injection mi ON mi.power_plant_id = mp.power_plant_id
    AND mi.year = mp.year AND mi.month = mp.month
JOIN monthly_total_withdrawal mtw ON mtw.year = mp.year AND mtw.month = mp.month
JOIN monthly_total_injection mti ON mti.year = mp.year AND mti.month = mp.month;
...
FROM (
    ...
    JOIN raforka_updated.station s1 ON s1.id = ct2.from_substation_id
    JOIN raforka_updated.station s2 ON s2.id = ct2.to_substation_id
) calc
WHERE ct.from_substation_id = calc.from_substation_id
  AND ct.to_substation_id   = calc.to_substation_id;

-- Flow estimation for a date range

WITH
s1_injection AS (
    SELECT SUM(i.value_kwh) AS total_kwh
    FROM raforka_updated.injects_to i
    JOIN raforka_updated.substation sub ON i.substation_id = sub.substation_id
    JOIN raforka_updated.station s ON sub.substation_id = s.id
...
s3_withdrawal AS (
    ...
    FROM raforka_updated.withdraws_from w
    JOIN raforka_updated.substation sub ON w.substation_id = sub.substation_id
    JOIN raforka_updated.station s ON sub.substation_id = s.id
...
distances AS (
    ...
    JOIN raforka_updated.substation sub_f ON ct.from_substation_id = sub_f.substation_id
    JOIN raforka_updated.station sf ON sub_f.substation_id = sf.id
),
flow AS (
    SELECT
...
-- Write back estimated flows
-- max_capacity_mw estimated with peak hourly flow
BEGIN;
-- S1 -> S2
WITH flow AS (
    SELECT
    ...
    FROM
        ...
        (SELECT
         ...
         FROM raforka_updated.connects_to ct
         JOIN raforka_updated.substation sub_f ON ct.from_substation_id = sub_f.substation_id
         JOIN raforka_updated.station sf ON sub_f.substation_id = sf.id) d
),
peak_capacity AS (
